...
# 🌟 新增：可摺疊的法律免責聲明（放在上傳按鈕上方，最為安全顯眼）
with st.expander("⚖️ 隱私保護與使用免責聲明（上傳前請必讀）", expanded=False):
    st.markdown("""
    * **零資料留存機制**：本工具為純前端即時解析軟體，本專案之原始碼**絕無編寫、亦不具備**任何資料庫上傳、硬碟儲存或歷史紀錄留存之 feature。
    * **即時銷毀**：上傳之 CSV 檔案僅短暫載入伺服器記憶體（RAM）中進行鏡射排版計算。網頁一旦重整、關閉或點擊「下一位」按鈕，數據將立刻從記憶體中完全釋放銷毀。
    * **去識別化建議**：由於本服務部署於第三方雲端平台（Streamlit Cloud），為求絕對安全，強烈建議使用者在上傳 Excel/CSV 前，**先將檔案內之病患敏感資訊（如真實姓名、身分證字號）修改或塗黑匿名化**。
    * **免責條款**：本工具僅供臨床輔助格式轉換與病歷速記使用，不構成醫療診斷。開發者對於第三方網路傳輸安全或因使用者操作導致之任何潛在風險，一概不負任何法律與賠償責任。上傳檔案即視同同意上述條款。
    """)
